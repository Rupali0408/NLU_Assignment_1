...
def backoff_probability(w, unigram, bigram, trigram, u_sum):
    p = 0.0
    t_sum = 0.0
    b_sum = 0.0
    alpha = 0.0
    ...
    if t_outer_key in trigram:
        for t_inner_key in trigram[t_outer_key]:
            value = trigram[t_outer_key][t_inner_key]
            t_sum += value[1]

    if b_outer_key in bigram:
        for b_inner_key in bigram[b_outer_key]:
...
def perplexity(test_data, unigram, bigram, trigram, unigram_prob_sum):
    ...
    for sentence in test_data:
        ...
        for i in range(len(sentence) - 2):
            ...
            p = p * backoff_probability(w, unigram, bigram, trigram, unigram_prob_sum)
        if p == 0.0:
            continue
        #print(count)
        Pw = (math.log(p, 2))
